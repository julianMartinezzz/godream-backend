    ...
    const exportarExcel = () => {
        const datosReporte = asesores.map(as => {
            ...
            return {
                ...
                "PLANES": instaladas.map(l => l.plan).join(", "),
                "TOTAL": comision
            };
        }).filter(i => i !== null);
        ...
        XLSX.writeFile(wb, `Pagos_${MESES[mesSeleccionado]}.xlsx`);
    };
                ...
                <div className="max-w-7xl mx-auto">
                    <div className="flex justify-between items-start mb-10">
                        ...
                        <div className="flex gap-4 items-center">
                            <select
                                ...
                                className="bg-white border-2 border-slate-100 p-3 rounded-2xl font-bold text-slate-700 outline-none focus:border-orange-500 shadow-sm"
                            >
                                {MESES.map((mes, index) => <option key={mes} value={index}>{mes}</option>)}
                            </select>
                            <button onClick={exportarExcel} className="bg-slate-900 text-white px-6 py-4 rounded-2xl font-black flex items-center gap-2 hover:bg-green-600 transition-all shadow-lg active:scale-95">
                            ...
                            <div className="bg-green-500 text-white px-8 py-4 rounded-[24px] shadow-lg flex items-center gap-3">
                                <Wallet size={24} />
                                <div>
                                    <p className="text-[10px] font-bold uppercase opacity-80">Total Periodo</p>
                                    <p className="text-2xl font-black">${asesores.reduce((acc, as) => acc + calcularComisionAsesor(as.id), 0).toLocaleString()}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
                        <div className="bg-white p-8 rounded-[32px] shadow-sm border border-slate-100">
                            ...
                            <div className="text-4xl font-black text-slate-900">{leadsDelPeriodo.filter(l => l.estado === 'INSTALADA').length}</div>
                        </div>
                        <div className="bg-white p-8 rounded-[32px] shadow-sm border border-slate-100 opacity-50">
                            ...
                            <div className="text-4xl font-black text-slate-900">{leadsDelPeriodo.filter(l => l.estado === 'PENDIENTE_INSTALACION').length}</div>
                        </div>
                        <div className="bg-white p-8 rounded-[32px] shadow-sm border border-slate-100 opacity-50">
                            ...
                            <div className="text-4xl font-black text-slate-900">{leadsDelPeriodo.filter(l => l.estado === 'CANCELADA').length}</div>
                        </div>
                    </div>
                                    ...
                                    <tr key={as.id} className="hover:bg-slate-50 transition-all">
                                        ...
                                        <td className="p-8">
                                            <div className="flex flex-wrap gap-2">
                                                {instaladas.map(l => <span key={l.id} className="px-3 py-1 bg-slate-100 rounded-lg text-[10px] font-bold text-slate-600 border border-slate-200">{l.plan}</span>)}
                                            </div>
                                        </td>
                                        <td className="p-8 text-right">
                                            <div className="bg-green-50 text-green-700 px-6 py-3 rounded-2xl font-black text-xl inline-block border border-green-100 shadow-sm">$ {comision.toLocaleString()}</div>
                                        </td>
                                    </tr>
                                );
                            })}
